...
make
```
to run the analysis all at once. This will execute our three steps: compile the `.qmd` files in `./src/data`, compile the `.qmd` files in `./src/analysis`, then finally compile the `.tex` files in `./tex`.
